                    ...
                    <select class="form-control" id="training_id" name="training_id" required>
                        @foreach ($trainings as $training)
                            <option value="{{ $training->id }}"
                                {{ $training->id == $partner_Training_Users->training_id ? 'selected' : '' }}>
                                {{ $training->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select class="form-control" id="partner_id" name="partner_id" required>
                        @foreach ($partners as $partner)
                            <option value="{{ $partner->id }}"
                                {{ $partner->id == $partner_Training_Users->partner_id ? 'selected' : '' }}
                        ...
                        @foreach ($users as $user)
                            @if ($user->role_id == 4)
                                <option value="{{ $user->id }}"
                                    {{ $user->id == $partner_Training_Users->user_id ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endif
                        @endforeach
                                ...
                                <tr>
                                    ...
                                    <td class="pl-5">
                                        <input type="checkbox" name="materials[{{ $material->id }}]"
                                            value="{{ $material->id }}"
                                            {{ $material->pivot->quantity > 0 ? 'checked' : '' }}>
                                    </td>
                                </tr>
                                <tr class="filler"></tr>
                            @endforeach
...
@section('scripts')
    ...
    <script type="module" src="{{ asset('js/external/edit.js') }}"></script>
@endsection
