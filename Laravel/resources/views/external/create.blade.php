...
@section('content')
    ...
    <div class="w-100">
        ...
        @error('start_date')
            <div class="alert alert-danger error-alert">{{ $message }}</div>
        @enderror
        ...
        <form method="POST" action="{{ url('external') }}" style="width: 100%">
            ...
            <div class="grid">
                ...
                <div class="partner">
                    ...
                    <select class="form-control" id="partner_id" name="partner_id" required>
                        @foreach ($partners as $partner)
                            <option value="{{ $partner->id }}" data-address="{{ $partner->address }}">{{ $partner->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                                ...
                                <tr>
                                    ...
                                    <td class="pl-4">
                                        <input type="number" name="material_quantities[{{ $material->id }}]"
                                            value="1" min="1" max="{{ $material->quantity }}"
                ...
                <div class="startCalendar">
                    ...
                    <input type="datetime-local" class="form-control flatpickr" id="start_date" name="start_date" required
                        placeholder="Selecione a data de início">
                </div>

                <div class="endCalendar">
                    <label for="end_date">Data de Fim:</label>
                    <input type="datetime-local" class="form-control flatpickr" id="end_date" name="end_date" required
...
@section('scripts')
    ...
    <script type="module" src="{{ asset('js/external/create.js') }}"></script>
@endsection
