                                ...
                                <select class="form-control w-auto" id="filterCategory" name="filterCategory"
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}"
                                            {{ (int) $filterCategory === $category->id ? 'selected' : '' }}>
                                ...
                                <select class="form-control w-auto" id="filterStatus" name="filterStatus"
                                    ...
                                    @foreach ($statuses as $status)
                                        <option value="{{ $status->id }}"
                                            {{ (int) $filterStatus === $status->id ? 'selected' : '' }}>
                                            {{ $status->description }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <table class="table bg-white rounded-top">
                            <thead>
                                <tr>
                                    <th scope="col">
                                        <a
                                            href="{{ route('tickets.index', ['sort' => 'number', 'direction' => $currentSort === 'number' ? $newDirection : 'asc']) }}">
                                            Número
                                        </a>
                                    </th>
                                    <th scope="col">
                                        <a
                                            href="{{ route('tickets.index', ['sort' => 'title', 'direction' => $currentSort === 'title' ? $newDirection : 'asc']) }}">
                                            Título
                                        </a>
                                    </th>
                                    <th scope="col">
                                        <a
                                            href="{{ route('tickets.index', ['sort' => 'user', 'direction' => $currentSort === 'user' ? $newDirection : 'asc']) }}">
                                            Utilizador
                                        </a>
                                    </th>
                                    <th scope="col">
                            ...
                            <tbody>
                                ...
                                @foreach ($tickets as $ticket)
                                    <tr class="customTableStyling {{ $ticket->ticketPriority->id == 5 ? 'critical' : '' }}"
                                        ...
                                        <td class="ticket-status-{{ $ticket->ticketStatus->id }}">
                                            {{ $ticket->ticketStatus->description ? $ticket->ticketStatus->description : 'N.A.' }}
                                        </td>
                            ...
                            <tbody>
                                ...
                                @foreach ($waitingQueueTickets as $ticket)
                                    <tr
                                        ...
                                        <td class="ticket-status-{{ $ticket->ticketStatus->id }}">
                                            {{ $ticket->ticketStatus->description }}</td>
